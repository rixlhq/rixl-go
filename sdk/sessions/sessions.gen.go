...
type LogoutJSONRequestBody = models.AuthV1LogoutRequest

type RefreshTokenJSONRequestBody = models.AuthV1RefreshTokenRequest
...
type ClientInterface interface {
	// LogoutWithBody makes a POST request to /auth/v1/logout
	...
	// RefreshTokenWithBody makes a POST request to /auth/v1/token
...
// Logout makes a POST request to /auth/v1/logout with application/json body
...
// RefreshToken makes a POST request to /auth/v1/token with application/json body
...
// NewLogoutRequest creates a POST request for /auth/v1/logout with application/json body
...
// NewLogoutRequestWithBody creates a POST request for /auth/v1/logout with any body
func NewLogoutRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/auth/v1/logout")
	if operationPath[0] == '/' {
...
// NewRefreshTokenRequest creates a POST request for /auth/v1/token with application/json body
...
// NewRefreshTokenRequestWithBody creates a POST request for /auth/v1/token with any body
func NewRefreshTokenRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/auth/v1/token")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
// Logout makes a POST request to /auth/v1/logout and returns the parsed response.
// Logout
// On success, returns the response body. On HTTP error, returns *ClientHttpError[struct{}].
func (c *SimpleClient) Logout(ctx context.Context, body LogoutJSONRequestBody, reqEditors ...RequestEditorFn) (models.GoogleProtobufEmpty, error) {
	var result models.GoogleProtobufEmpty
	resp, err := c.Client.Logout(ctx, body, reqEditors...)
	if err != nil {
		return result, err
	}
...
// RefreshToken makes a POST request to /auth/v1/token and returns the parsed response.
// RefreshToken
// On success, returns the response body. On HTTP error, returns *ClientHttpError[struct{}].
func (c *SimpleClient) RefreshToken(ctx context.Context, body RefreshTokenJSONRequestBody, reqEditors ...RequestEditorFn) (models.AuthV1TokenResponse, error) {
	var result models.AuthV1TokenResponse
	resp, err := c.Client.RefreshToken(ctx, body, reqEditors...)
	if err != nil {
		return result, err
	}
	defer resp.Body.Close()
